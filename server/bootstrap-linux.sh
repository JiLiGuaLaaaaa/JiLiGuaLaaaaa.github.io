...
ensure_node_runtime() {
  ...
  if command -v apt-get >/dev/null 2>&1 && command -v curl >/dev/null 2>&1; then
    ...
    curl -fsSL https://deb.nodesource.com/setup_22.x -o /tmp/blog-dynamic-nodesource.sh
    bash /tmp/blog-dynamic-nodesource.sh
    rm -f /tmp/blog-dynamic-nodesource.sh
  ...
  node_bin="${BLOG_DYNAMIC_NODE_BIN:-$(command -v node || true)}"
  [ -n "$node_bin" ] || fail "Node.js 18+ is required. Install Node.js manually and rerun this script."

  node_major="$("$node_bin" -p "Number(process.versions.node.split('.')[0])" 2>/dev/null || printf '0')"
  if [ "$node_major" -lt 18 ]; then
    fail "Node.js 18+ is required. Current node is $("$node_bin" -v 2>/dev/null || printf 'unknown')."
  fi
  NODE_BIN="$node_bin"
}
