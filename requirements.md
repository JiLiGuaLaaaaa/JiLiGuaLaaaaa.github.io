...
用户本次明确要求使用阿里云 ECS 为博客补充动态能力，但公开博客本体仍继续使用 GitHub Pages 托管静态页面。

新增动态能力必须作为独立服务实现：

- 静态博客继续由 Astro 构建并发布到 GitHub Pages。
- ECS 运行单独的动态服务，用于日记、生活记录和访问量统计等 API。
- 静态博客通过可配置的 API 基础地址访问 ECS 动态服务。
...
- 不在仓库中写入真实 ECS 公网 IP、真实 SSH 用户、真实密码、证书私钥或 Cloudflare API Token。
