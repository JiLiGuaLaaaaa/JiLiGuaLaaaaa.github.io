...
- `video-mascot/look-angle/`：每 10° 一张的 36 张方向细分帧。现在优先从原视频连续动作段抽取稳定真实帧，缺少的右侧对称角度使用真实帧镜像生成，不再使用透明叠加、权重混合或插值补帧，避免出现双脸、双头发或半透明残影。组件会根据鼠标相对角度选择最接近的方向帧，例如上偏左、上偏右、下偏左、下偏右等。
