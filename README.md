...
- 主题 Banner：博客、标签、归档、搜索、关于页面使用 `public/images/banner/` 中按主题命名的图片；Banner 前景图会在保证主要人物完整可见的前提下尽可能占满整个区域，只有图片比例无法铺满时才由模糊背景层补足空白，并由全站脚本在向下滚动后统一流畅收起。正文较短的页面会保留额外空白滚动区，保证 Banner 仍然可以被收起。
