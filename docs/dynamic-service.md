...
- 写日记入口：连续点击小人多次后打开动态服务的管理入口。
- 安全边界：小人入口只是隐藏入口，真正写入由动态服务的认证保护。
...
- `GET /api/life?limit=6`：读取已发布的公开生活记录。

管理 API：

- `POST /api/diary`：写入私密日记。
- `GET /api/diary`：读取私密日记列表。
- `POST /api/life`：创建生活记录，可保存为草稿或发布为公开记录。
...
- `stats.json`：访问量聚合数据。
- `life-records.json`：生活记录，只有 `status: "published"` 会被公开 API 返回。
