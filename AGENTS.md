...
如果以后需要私密日记，应作为单独服务部署在独立动态服务服务器上，并加访问控制。
...
- 动态能力只能作为独立服务部署到独立动态服务服务器，不把后端运行时混入 GitHub Pages。
...
- 域名配置优先使用子域名隔离静态站和动态服务，例如公开博客继续使用主博客域名，动态服务使用单独 API 子域名；实际记录以本地配置和用户确认为准。
